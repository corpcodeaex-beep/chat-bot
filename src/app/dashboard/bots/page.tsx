...
export default async function AllBotsPage() {
  await pageAdmin();
  const [bots, stats, companies] = await Promise.all([listBots(), getStats(), listClients()]);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">Every bot across all companies. Open one to manage it exactly like the company does.</p>
        </div>
      ...
      {bots.length === 0 ? (
        ...
        <BotCards bots={bots} perBot={stats.perBot} companyNames={companyNames} />
      )}
    </div>
  );
}
